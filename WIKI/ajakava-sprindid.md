# Sprintide ajakava

Jaga projekt väiksemateks tööperioodideks ehk sprintideks.

## Sprint 1

### Periood
- Algus: september
- Lõpp: november

### Eesmärk
Koostada teoreetiline osa ning luua persoonad ja stsenaariumid.

### Peamised ülesanded
- [ ] Teooria uurimine ja kirjutamine (veebileht, kasutatavus, struktuur)
- [ ] Sihtrühma analüüs
- [ ] Persoonade loomine
- [ ] Stsenaariumite koostamine
- [ ] Allikate lisamine

### Vastutajad
- Linda (sisulooja)
- Markus (projektijuht)

### Tulemus
Valmis teooria, persoonad ja stsenaariumid.

### Märkused
- Täidetakse töö käigus

## Sprint 2

### Periood
- Algus: detsember
- Lõpp: jaanuar

### Eesmärk
Luua esmane prototüüp ja planeerida veebilehe struktuur.

### Peamised ülesanded
- [ ] Nõuete kaardistamine
- [ ] Veebilehe struktuuri planeerimine
- [ ] Paberprototüübi loomine
- [ ] Tagasiside kogumine ja parandused

### Vastutajad
- Linda (disainer)
- Kaisa (arendaja)

### Tulemus
Valmis esmane prototüüp ja veebilehe struktuur.

### Märkused
- Täidetakse töö käigus.

## Sprint 3

### Periood
- Algus: veebruar
- Lõpp: märts

### Eesmärk
Luua valmis lahenduse prototüüp ja arendada veebileht.

### Peamised ülesanded
- [ ] Veebilehe disaini loomine
- [ ] Lehtede loomine valitud keskkonnas
- [ ] Sisu kirjutamine ja lisamine
- [ ] Registreerimisvormi loomine
- [ ] Testimine ja parandused

### Vastutajad
- Kaisa (arendaja)
- Linda (disainer/sisulooja)

### Tulemus
Valmis toimiva veebilehe prototüüp.

### Märkused
- Täidetakse töö käigus

## Sprint 4

### Periood
- Algus: aprill
- Lõpp: mai

### Eesmärk
Valmistuda digiloovtöö kaitsmiseks ja viimistleda projekt.

### Peamised ülesanded
- [ ] Veebilehe lõplik viimistlus
- [ ] Dokumentatsiooni täiendamine
- [ ] Esitluse (slaidid) koostamine
- [ ] Kaitsmise harjutamine

### Vastutajad
- Markus (projektijuht/testija)
- Kaisa (arendaja)
- Linda (sisulooja)

### Tulemus
Valmis esitlus ja kaitsmiseks valmis projekt.

### Märkused
- Täidetakse töö käigus
